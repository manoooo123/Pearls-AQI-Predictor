...
/* Streamlit Header / Menu Reset & Native Sidebar Controls */
...
div.st-key-btn_toggle_sidebar_hide button,
div[data-testid="stElementContainer"]:has(button[key="btn_toggle_sidebar_hide"]) button {{
    background: transparent !important;
    border: 1px solid {border_color} !important;
    color: {text_sec} !important;
...
div.st-key-btn_toggle_sidebar_hide button:hover,
div[data-testid="stElementContainer"]:has(button[key="btn_toggle_sidebar_hide"]) button:hover {{
    background: {bg_card_sec} !important;
    color: {brand_pri} !important;
    border-color: {brand_pri} !important;
}}

/* Internal Scrollbar for Dataframes & Tables on Small Viewports */
...
if st.session_state["user"] is None:
    ...
    with c_left:
        st.markdown("""
        <div class="industry-card" style="padding:36px 30px; min-height:510px; display:flex; flex-direction:column; justify-content:space-between;">
            <div>
                <div style="display:flex; align-items:center; gap:12px; margin-bottom:18px;">
                    <span style="font-size:36px;">⚡</span>
                    <h1 style="margin:0; font-size:26px; font-weight:700; letter-spacing:-0.3px;">Pearls AQI Predictor</h1>
                </div>
                <div style="font-size:17px; font-weight:600; color:#10B981; margin-bottom:14px; letter-spacing:-0.2px;">
                    Breathe smarter. Predict cleaner.
                </div>
                <p style="font-size:14px; line-height:1.65; color:#60736B; margin-bottom:26px;">
                    Enterprise-grade environmental AI platform offering multi-horizon (24h, 48h, 72h) air quality predictions, live OpenAQ v3 sensor ingestion, and SHAP feature explainability.
                </p>
                <div style="display:flex; flex-direction:column; gap:12px; background:rgba(27,77,62,0.04); padding:18px 16px; border-radius:12px; border:1px solid #DCE6E0;">
                    <div style="font-size:11.5px; font-weight:700; text-transform:uppercase; letter-spacing:0.5px; color:#8E9F97;">Active Operational Nodes</div>
                    <div style="display:flex; justify-content:space-between; align-items:center; font-size:13px; font-weight:600;">
                        <span>📍 Lahore</span>
                        <span class="status-badge badge-good">Active Monitoring</span>
                    </div>
                    <div style="display:flex; justify-content:space-between; align-items:center; font-size:13px; font-weight:600;">
                        <span>📍 Islamabad</span>
                        <span class="status-badge badge-good">Active Monitoring</span>
            ...
            <div style="font-size:12px; color:#8E9F97; margin-top:28px; border-top:1px solid rgba(0,0,0,0.05); padding-top:16px;">
                🔒 Secure PBKDF2-HMAC Password Hashing & Authenticated Session Tokens
            </div>
        </div>
    ...
    with c_right:
        ...
        with tab_login:
            ...
            with st.form("auth_portal_login_form"):
                email_in = st.text_input("Email Address", key="portal_login_email", placeholder="name@company.com")
                pwd_in = st.text_input("Password", type="password", key="portal_login_pwd", placeholder="••••••••")
                submit_login = st.form_submit_button("Sign In to Platform", use_container_width=True)

                if submit_login:
                    if not email_in or not pwd_in:
                        st.error("Authentication Error: Please provide both your registered email address and password.")
                    else:
                        ok, msg, user_obj = authenticate_user(email_in.strip(), pwd_in)
                        if ok and user_obj:
                            token = create_session(user_obj["id"])
                            st.session_state["user"] = user_obj
                            st.session_state["auth_token"] = token
                            st.session_state["logged_out"] = False
                            st.success("Authenticated successfully! Loading platform...")
                            st.rerun()
                        else:
                            st.error(msg or "Invalid email address or password.")

        with tab_reg:
            st.markdown("<div style='height:10px;'></div>", unsafe_allow_html=True)
            with st.form("auth_portal_reg_form"):
                ...
                if pwd_reg:
                    ...
                    st.markdown(f"""
                    <div style="margin-top:-6px; margin-bottom:10px; font-size:12px; font-weight:600; color:{col};">
                        Password Strength: {label} ({score}%)
                    </div>
                    """, unsafe_allow_html=True)

                pwd_conf = st.text_input("Confirm Password", type="password", key="portal_reg_conf", placeholder="••••••••")
                submit_reg = st.form_submit_button("Create Account", use_container_width=True)

                if submit_reg:
                    if not email_reg or not pwd_reg or not name_in:
                        st.error("Registration Error: All fields are required.")
                    elif pwd_reg != pwd_conf:
                        st.error("Registration Error: Passwords do not match.")
                    elif len(pwd_reg) < 8:
                        st.error("Registration Error: Password must be at least 8 characters long.")
                    else:
                        ok, msg, new_uid = register_user(email_reg.strip(), pwd_reg, name_in.strip())
                        if ok:
                            token = create_session(new_uid)
        ...
        if st.button("🌐 Continue as Guest (Read-Only Mode)", use_container_width=True):
            st.session_state["user"] = {
                "id": 0,
                "email": "guest@pearlsaqi.com",
...
prefs = get_user_preferences(user["id"]) if user.get("id") else {}
