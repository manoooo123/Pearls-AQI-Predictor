...
class AuditLogger:
    ...
    @staticmethod
    def log_session_invalidated(token: Optional[str] = None, reason: str = "logout"):
        """Log session invalidation."""
        token_str = (token[:8] if token else "none")
        logger.info(f"SESSION INVALIDATED | Token: {token_str}... | Reason: {reason}")
